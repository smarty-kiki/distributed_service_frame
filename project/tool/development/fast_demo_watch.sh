        ...
        (
            ...
            do
                ...
                if [ "$event" = "CREATE" ] || [ "$event" = "MODIFY" ];then
                    ...
                    echo delete $ROOT_DIR/service/$entity_name.php success!

                    ENV=$env /usr/bin/php $ROOT_DIR/public/cli.php crud:make-from-description --entity_name=$entity_name
                    /bin/sed -i "/init\ service/a\include\ CONTROLLER_DIR\.\'\/$entity_name\.php\'\;" $ROOT_DIR/public/index.php
                    echo include $ROOT_DIR/service/$entity_name.php success!
                fi
