...
define('MIGRATION_DATABASE_CONFIG_KEY', unit_of_work_db_config_key());
...
{/*{{{*/
    db_structure(
        'CREATE TABLE IF NOT EXISTS `'.MIGRATION_TABLE.'` (
            ...
            PRIMARY KEY (`id`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci', MIGRATION_DATABASE_CONFIG_KEY);
});/*}}}*/

command('migrate:uninstall', '删除 migrate 所需的表结构', function ()
{/*{{{*/
    db_structure('DROP TABLE `'.MIGRATION_TABLE.'`', MIGRATION_DATABASE_CONFIG_KEY);
});/*}}}*/

command('migrate', '执行 migrate', function ()
{/*{{{*/
    $files = scandir(MIGRATION_DIR);
    $old_migrations = array_merge(['.', '..', '.gitkeep'], db_query_column('migration', 'select * from '.MIGRATION_TABLE, [], MIGRATION_DATABASE_CONFIG_KEY));
    $new_migrations = array_diff($files, $old_migrations);

    $last_batch = db_query_value('max_batch', 'select max(batch) max_batch from '.MIGRATION_TABLE, [], MIGRATION_DATABASE_CONFIG_KEY);

    foreach ($new_migrations as $filename) {
        list($ups, $downs) = _migration_file_explode(MIGRATION_DIR.'/'.$filename);

        foreach ($ups as $up) {
            db_structure($up, MIGRATION_DATABASE_CONFIG_KEY);
        }

        db_insert('insert into '.MIGRATION_TABLE.' set migration = :migration, batch = :batch', [
            ':migration' => $filename,
            ':batch' => $last_batch + 1,
        ], MIGRATION_DATABASE_CONFIG_KEY);
...
{/*{{{*/
    ...
    foreach ($last_batch_migrations as $filename) {
        ...
        foreach ($downs as $down) {
            db_structure($down, MIGRATION_DATABASE_CONFIG_KEY);
        }
...
{/*{{{*/
    $migrations = db_query_column('migration', 'select migration from '.MIGRATION_TABLE.' order by id desc', [], MIGRATION_DATABASE_CONFIG_KEY);
    ...
    foreach ($migrations as $filename) {
        ...
        db_delete('delete from '.MIGRATION_TABLE, [], MIGRATION_DATABASE_CONFIG_KEY);
