...
define('DESCRIPTION_SERVICE_EXTENSION_DIR', DESCRIPTION_EXTENSION_DIR.'/service');
...
function _get_data_type_service_from_extension($action, $data_type)
{/*{{{*/
    $path = DESCRIPTION_SERVICE_EXTENSION_DIR.'/'.$action.'/data_type/'.$data_type.'.php';
    if (is_file($path)) {
...
function _get_service_template_from_extension($action)
{/*{{{*/
    $path = DESCRIPTION_SERVICE_EXTENSION_DIR.'/'.$action.'/service.php';
    if (is_file($path)) {
        return file_get_contents($path);
    }
